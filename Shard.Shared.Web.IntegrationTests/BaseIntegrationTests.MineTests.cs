...
namespace Shard.Shared.Web.IntegrationTests
{
    public partial class BaseIntegrationTests<TEntryPoint, TWebApplicationFactory>
    {
        ...
        public async Task FetchingBuildingsOfWrongUserReturns404()
        {
            ...
            var response = await client.GetAsync($"{originalBuilding.UserPath}z/buildings");
            await response.AssertStatusEquals(HttpStatusCode.NotFound);
        }

        private static async Task<Building> RefreshBuilding(HttpClient client, Building originalBuilding)
        {
            var response = await client.GetAsync(originalBuilding.Url);
            await response.AssertSuccessStatusCode();

            return new Building(originalBuilding.UserPath, await response.AssertSuccessJsonAsync());
        }
        ...
        public async Task FetchingOneBuildingOfWrongUserReturns404()
        {
            ...
            var response = await client.GetAsync($"{originalBuilding.UserPath}z/buildings/{originalBuilding.Id}");
            await response.AssertStatusEquals(HttpStatusCode.NotFound);
        }
        ...
        public async Task FetchingOneBuildingWithWrongIdReturns404()
        {
            ...
            var response = await client.GetAsync($"{originalBuilding.UserPath}/buildings/{originalBuilding.Id}z");
            await response.AssertStatusEquals(HttpStatusCode.NotFound);
        }
        ...
        public async Task GetMine_IfMoreThan2secBeforeBuilt_DoesNotWait()
        {
            ...
            var responseTask = RefreshBuilding(client, originalBuilding);
        ...
        public async Task GetMine_IfLessOrEqualThan2secBeforeBuilt_WaitsUntilUnitMoves_AndReturns404()
        {
            ...
            var responseTask = client.GetAsync(originalBuilding.Url);
            await Task.Delay(500);
        ...
        {
            using var client = CreateClient();
            var originalBuilding = await BuildMineOn(client, "13ed60e3-1692-56cd-ae38-b7c02013ce9e", "9e466528-42f7-2279-32cd-f5f595113d17");

            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 20);

            await fakeClock.Advance(TimeSpan.FromMinutes(6));

            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 21);
        }
        ...
        public async Task BuildingMineThenWaitingLotsOfTimeIncreaseResourceByUpToThePlanetResources()
        {
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 687 + 20);
        }
        ...
        {
            using var client = CreateClient();
            var originalBuilding = await BuildMineOn(client, "13ed60e3-1692-56cd-ae38-b7c02013ce9e", "5915a33d-8c25-105b-56d4-10de1a2ab3fe");
            await fakeClock.Advance(TimeSpan.FromMinutes(5));
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 20 + 35);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "iron", 10);
        }
        ...
        public async Task MineExtractThenMostRareResourceAlternating()
        {
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 20 + 35);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "iron", 10 + 1);

            await fakeClock.Advance(TimeSpan.FromMinutes(1));

            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 20 + 36);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "iron", 10 + 1);
        }
        ...
        public async Task MineCanExhaustPlanet()
        {
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "carbon", 20 + 63);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "iron", 10 + 28);
        }
        ...
        {
            using var client = CreateClient();
            var originalBuilding = await BuildMineOn(client, "13ed60e3-1692-56cd-ae38-b7c02013ce9e", "3697bdfb-adaf-a820-018b-854cd49ff686",
                "solid");
            await fakeClock.Advance(TimeSpan.FromMinutes(5));

            await AssertResourceQuantity(client, originalBuilding.UserPath, "aluminium", 0);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "water", 50);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "oxygen", 50);

            // There are 35 more carbon than iron
            await fakeClock.Advance(TimeSpan.FromMinutes(1000));

            await AssertResourceQuantity(client, originalBuilding.UserPath, "aluminium", 64);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "water", 50);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "oxygen", 50);
        }
        ...
        public async Task LiquidMineExtractOnlyWater()
        {
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "aluminium", 0);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "water", 50 + 15);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "oxygen", 50);
        }
        ...
        public async Task GasMineExtractOnlyOxygen()
        {
            ...
            await AssertResourceQuantity(client, originalBuilding.UserPath, "aluminium", 0);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "water", 50);
            await AssertResourceQuantity(client, originalBuilding.UserPath, "oxygen", 50 + 113);
        }
    }
}
